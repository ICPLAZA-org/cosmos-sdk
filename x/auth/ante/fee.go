...
func burnFees(bankKeeper types.BankKeeper, ctx sdk.Context, acc types.AccountI, fees sdk.Coins, burntFeeDenom string) (sdk.Coins, error) {
	...
	if amount.IsPositive() {
		...
		feesL := sdk.NewCoin(burntFeeDenom, amount)

		if err := bankKeeper.SendCoinsFromAccountToModule(ctx, acc.GetAddress(), types.BurntFeeCollectorName, sdk.NewCoins(feesL)); err != nil {

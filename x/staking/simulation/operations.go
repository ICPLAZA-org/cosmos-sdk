...
func SimulateMsgCreateValidator(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		teamAccount,_ := simtypes.RandomAcc(r, accs)

		msg, err := types.NewMsgCreateValidator(address, simAccount.ConsKey.PubKey(), selfDelegation, description, 
			commission, reallocatedCommissionRule, teamAccount.Address, sdk.OneInt())
		if err != nil {
...
func SimulateMsgEditValidator(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		msg := types.NewMsgEditValidator(address, sdk.AccAddress(address).String(), description, &newCommissionRate, nil)
...
func SimulateMsgDelegate(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		recommanderAccount, _ := simtypes.RandomAcc(r, accs)
		...
		msg := types.NewMsgDelegate(simAccount.Address, val.GetOperator(), recommanderAccount.Address, bondAmt)

...
// getMaxIncentiveDepth returns randomized MaxIncentiveDepth
func getMaxIncentiveDepth(r *rand.Rand) (maxIncentiveDepth uint32) {
	return uint32(r.Intn(15) + 1)
}
...
func RandomizedGenState(simState *module.SimulationState) {
	...
	simState.AppParams.GetOrGenerate(
		simState.Cdc, historicalEntries, &histEntries, simState.Rand,
		func(r *rand.Rand) { maxIncentiveDepth = getMaxIncentiveDepth(r) },
	)

	// NOTE: the slashing module need to be defined after the staking module on the
	// NewSimulationManager constructor for this to work
	simState.UnbondTime = unbondTime
	params := types.NewParams(simState.UnbondTime, maxVals, 7, histEntries, maxIncentiveDepth, sdk.DefaultBondDenom)
	...
	for i := 0; i < int(simState.NumBonded); i++ {
		...
		reallocatedCommissionRule := types.NewReallocatedCommissionRule(
			simulation.RandomDecAmount(simState.Rand, maxCommission),
			simulation.RandomDecAmount(simState.Rand, maxCommission),
			uint32(0),
			[]types.RecommanderClassRate{},
		)
		...
		validator.ReallocatedCommissionRule = reallocatedCommissionRule

		delegation := types.NewDelegation(simState.Accounts[i].Address, valAddr, sdk.AccAddress(valAddr), sdk.NewDecFromInt(simState.InitialStake))

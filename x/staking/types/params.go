...
var (
	...
	KeyMaxIncentiveDepth = []byte("MaxIncentiveDepth")
...
func NewParams(unbondingTime time.Duration, maxValidators, maxEntries, historicalEntries uint32, maxIncentiveDepth uint32, bondDenom string) Params {
	return Params{
		UnbondingTime:     unbondingTime,
		MaxValidators:     maxValidators,
		MaxEntries:        maxEntries,
		HistoricalEntries: historicalEntries,
		MaxIncentiveDepth: maxIncentiveDepth,
		BondDenom:         bondDenom,
	}
}
...
func (p Params) Validate() error {
	...
	if err := validateMaxIncentiveDepth(p.MaxIncentiveDepth); err != nil {
		return err
	}
...
func validateMaxIncentiveDepth(i interface{}) error {
	v, ok := i.(uint32)
	if !ok {
		return fmt.Errorf("invalid parameter type: %T", i)
	}

	if v > DefaultMaxIncentiveDepth {
		return fmt.Errorf("max must less than default value: %d <= %d", v, DefaultMaxIncentiveDepth)
	}

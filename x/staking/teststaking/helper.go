...
type Helper struct {
	...
	ReallocCommissionRule stakingtypes.ReallocatedCommissionRule
	// Coin Denomination
	Denom string
}
...
func NewHelper(t *testing.T, ctx sdk.Context, k keeper.Keeper) *Helper {
	return &Helper{t, staking.NewHandler(k), k, ctx, ZeroCommission(), ZeroReallocCommission(), sdk.DefaultBondDenom}
}
...
func (sh *Helper) CreateValidatorMsg(addr sdk.ValAddress, pk cryptotypes.PubKey, stakeAmount sdk.Int) *stakingtypes.MsgCreateValidator {
	coin := sdk.NewCoin(sh.Denom, stakeAmount)
	msg, err := stakingtypes.NewMsgCreateValidator(addr, pk, coin, stakingtypes.Description{}, sh.Commission, sh.ReallocCommissionRule, sdk.AccAddress(addr), sdk.OneInt())
...
// ZeroReallocCommission constructs a commission rates with all zeros.
func ZeroReallocCommission() stakingtypes.ReallocatedCommissionRule {
	return stakingtypes.NewReallocatedCommissionRule(sdk.ZeroDec(), sdk.ZeroDec(), 2, []stakingtypes.RecommanderClassRate{})
}
